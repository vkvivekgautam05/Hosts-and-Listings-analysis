select * from df_toronto_availability;  --5627197

select * from host_toronto_df;  --9975

select * from listing_toronto_df;  --15418

select * from review_toronto_df;  --400886
...
select * 
from host_toronto_df
where host_has_profile_pic = 'True'; --9883


--Identity verified
select * 
from host_toronto_df
where host_identity_verified = 'True';  --7967

--Is superhost
select * 
from host_toronto_df
where host_is_superhost = 'True';  --2353


--Review Score
select a.listing_id, b.name, count(*) as N_reviews
from review_toronto_df a
join listing_toronto_df b
on a.listing_id = b.id
group by a.listing_id, b.name
order by count(*) desc;                           --11981
...
join listing_toronto_df b
on a.listing_id = b.id
join host_toronto_df c
on b.host_id = c.host_id
where a.comments like '%great%' or a.comments like '%wonderful%' or a.comments like '%loved%'; --200092
...
join host_toronto_df c
on b.host_id = c.host_id
where a.comments like '%bad%'; --2230
...
from listing_toronto_df a
join host_toronto_df b
on a.host_id = b.host_id
